...
class Driver:
    ...
    def randInit():
        ...
        return Driver(numpy.random.rand(inputs,outputs)*2 - 1)
    ...
    def think(self, inputs):
        # add dummy input for bias weight
        inputs.insert(0,1)
        ...
        outputs = numpy.around(numpy.apply_along_axis(Driver.sigmoid,0,dotProducts))
        return outputs
...
def crossover(pair):
    ...
    for row in range(len(pair[0].weights)):
        ...
        for col in range(split,len(pair[0].weights[0])):
            weightRow.append(pair[1].weights[row][col])
    ...
    print(split)
    input()
    weights = numpy.concatenate((pair[0].weights[:split], pair[1].weights[split:]),axis=0)
...
def mutate(genome):
    genome.weights += (numpy.random.rand(genome.weights.shape[0],genome.weights.shape[1]) - 0.5) * .05
    return genome
...
def getCarSensors(carRect, car, game):
    ...
    for angle in car.sensors:
        ...
        for sensorDistance in range(car.sensorDistance):
            ...
            sensorPoint = (pos[0] + math.cos(angle/57.3)* sensorDistance, pos[1] - math.sin(angle/57.3)* sensorDistance)
...
def gameInit():
    ...
    game = loadJsonCourse("../courses/7.json")
...
def gameMain(driver, game, screen, assets):
    ...
    while running:
        ...
        car.outputs = driver.thinkReg(sensorVals)
        car.move()
        ...
        deltaDistance = euclidDistance((car.xPos,car.yPos),(lastCar.xPos, lastCar.yPos))
        
        totalDistance += deltaDistance

        lastDistance = distance

        fitness = assessFitness(game.wayPointIndex, elapsedTime,distance,totalDistance, car.speed)
        lastFitness = fitness
...
def main(argv):
    ...
    if len(argv) >= 2 and argv[0] == '-f':
        ...
        weightsFile ='bestWeights.json'
        generation = createInitialPopulation(10)
