...
class Car:
    ...
    def move(self):
        ...
        self.speed += max(min(self.outputs[1], 1), -1.5) * self.acceleration
...
class Driver:
    ...
    def think(self, inputs):
        ...
        for layer in self.weights:
            ...
            dotProducts = numpy.around(numpy.apply_along_axis(Driver.sigmoid, 0, dotProducts))
        outputs = dotProducts
        return outputs
...
def crossover(pair):
    ...
    #weights = [numpy.array(w) for w in weights]
...
def main(argv):
    ...
    if len(argv) >= 2 and argv[0] == '-f':
        ...
        weightsFile = 'bestWeightsReg.json'
